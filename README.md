...
- **多维度统计分析**:
    ...
    * 清晰展示总抽数、抽卡时间范围，以及当前距离上一个限定（UR）和SSR的垫抽数。
    * 精准计算获得限定（UR）和SSR的 **平均抽数**。
    * 一目了然的“最欧”和“最非”记录，展示获取限定（UR）或SSR所用的最少与最多抽数。
- **可视化抽卡历史**:
    * **出金历史轴**: 以进度条的形式直观展示每次获得限定（UR）或SSR的抽数，颜色会根据接近保底的程度（欧非程度）进行变化。
