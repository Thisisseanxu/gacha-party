...
import { isAdminUser, jsonResponse, verifyLicenseForEdgeOne } from './gacha-records.js'
...
export async function hashPassword(password, salt = '') {
  const hashBuf = await crypto.subtle.digest('SHA-256', textEncoder.encode(`${salt}${password}`))
...
export async function signAdminToken(env, userId) {
  ...
  const payloadB64 = base64UrlFromBytes(
    textEncoder.encode(
      JSON.stringify({ userId: String(userId), iat: now, exp: now + 24 * 60 * 60 * 1000 }),
    ),
  )
...
export async function requireAdmin(request, env) {
  ...
  if (!admin) return { response: jsonResponse({ message: '未授权，请重新登录' }, 401) }
  return { admin }
}
