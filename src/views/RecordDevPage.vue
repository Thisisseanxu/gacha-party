    ...
    <main class="dev-shell">
      ...
      <section class="tool-grid">
        <article class="panel">
          <div class="panel-title">
            <h2>下载记录</h2>
            <span class="cooldown-text">{{ downloadCooldownText }}</span>
          </div>
        ...
        <article class="panel">
          <div class="panel-title">
            <h2>上传记录</h2>
            <span class="cooldown-text">{{ uploadCooldownText }}</span>
          </div>
...
const downloadCooldownText = computed(() => cooldownText(DOWNLOAD_COOLDOWN_KEY))
const uploadCooldownText = computed(() => cooldownText(UPLOAD_COOLDOWN_KEY))
...
function cooldownText(key) {
  ...
  return timeLeft > 0 ? `冷却 ${millisecondsToTime(timeLeft)}` : '可请求'
}
...
async function downloadRecord() {
  ...
  try {
    ...
    saveJsonFile(wrappedJson, `gacha-record-${id}-${formatDate(new Date())}.json`)
...
async function buildUploadPayload(file, id) {
  ...
  try {
    ...
    throw new Error(`JSON 文件无法解析：${error.message}`)
  }
...
function validateCompressedRecord(base64Data, id) {
  try {
    ...
    throw new Error(`压缩记录无法解析：${error.message}`)
  }
}
